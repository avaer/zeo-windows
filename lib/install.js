...
const createLocalServer = ({name}) => {
  ...
  const _startServer = () => pidify(path.join(dirname, 'server.pid')).spawn(
    ...
    [
      '-NonInteractive',
      '-Command',
      `echo (Start-Process -FilePath ${path.join(dirname, 'windows', 'node', 'node.exe')} -ArgumentList index.js,noTty -NoNewWindow -RedirectStandardOutput out-log.txt -RedirectStandardError err-log.txt -PassThru).Id`
    ], {
      cwd: dirname,
      // detached: true,
      // stdio: 'ignore'
      ...
      cp.on('exit', code => {
        if (code === 0) {
          ...
          if (!isNaN(pid)) {
            ...
            reject(new Error('error parsing start server process output: ' + JSON.stringify(s)));
          }
        } else {
          reject(new Error('server start exited with nonzero status code: ' + code));
        }
      });
    }));
