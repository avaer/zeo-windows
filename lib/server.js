...
const tailShell = require('tail-shell');
...
const createLogStream = ({name}) => tailShell.createReadStream(path.join(__dirname, '..', 'servers', name, 'log.txt'), {tail: 200});
...
const _getNextPort = () => new Promise((accept, reject) => {
  const serversPath = path.join(__dirname, '..', 'servers');
  ...
  fs.readdir(serversPath, (err, files) => {
    if (!err || err.code === 'ENOENT') {
      ...
      Promise.all(files.map(server => new Promise((accept, reject) => {
        ...
        fs.readFile(installPidfilePath, (err, s) => {
          if (!err) {
            ...
            if (j !== null) {
              ...
              const err = new Error('failed to get server port from install pidfile: ' + installPidfilePath);
              err.code = 'EPARSE';
              reject(err);
            } else {
              const err = new Error('failed to parse install pidfile: ' + installPidfilePath);
        ...
        .then(ports => {
          const _recurse = (port = 8000) => {
            if (port < 10000) {
              if (!ports.includes(port)) {
                getport(port, (err, newPort) => {
                  if (!err) {
                    if (newPort === port) {
                      accept(port);
                    } else {
              ...
              const err = new Error('no free ports');
              err.code = 'ENOENT';
              reject(err);
            }
          };
